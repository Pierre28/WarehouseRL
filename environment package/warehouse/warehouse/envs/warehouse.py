...
class Warehouse(gym.Env):
    ...
    def __init__(self, N=3, render = False, verbose = 1):
        ...
        # 1) Fill randomly 10% of the spots with generate_package
    ...
    def step(self, action):
        ...
        if self.filling_ratio() >= min_filling_ratio :
            for _ in range(np.random.randint(33, 34)):
    ...
    def compute_max_access_cost(self, rep=10):
        '''
        Compute the maximum dollar equivalent energy required to reach a sequence of spots.
        Samples packages from the distribution to fill the warehouse at min_filling_ratio, and compute the cost to access the innermost package
        As a proxy, we consider every package has to move = all shelves moving to compute the maximum access cost
        '''

        energy_cost = 0
        for _ in range(rep):
            sample_pack = generate_packages(int(self.n_spots*min_filling_ratio))
            total_weight = sum(package.weight for package in sample_pack)
            energy_cost += energy_cost_fn(total_weight)

        return energy_cost/rep
