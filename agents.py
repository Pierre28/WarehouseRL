...
class MaskingDQNAgent(AbstractDQNAgent):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
        ...
        # Sample states to create Q-value based metric
        observations = []
        env.reset()
        for _ in range(100):
            for s in range(np.random.randint(1, 20)):
                _observation = []
            ...
            while self.step < nb_steps:
                ...
                if done:
                    ...
                    for observation in observations:
                        state = self.memory.get_recent_state(observation)
                        q_values = self.compute_q_values(state)
                        Q_values.append(max(q_values))
                        Q_min_values.append(min(q_values))
                    avg_Q_values = np.mean(Q_values)
                    avg_min_Q_values = np.mean(Q_min_values)
                    ...
                    episode_logs = {
                        ...
                        'nb_steps': self.step,
                        'episode_avg_maxq':avg_Q_values,
                        'episode_avg_minq': avg_min_Q_values
                    }
...
class CustomDQNAgent(AbstractDQNAgent):
    ...
    # Arguments
        model__: A Keras model.
        policy__: A Ker`as-rl policy that are defined in [policy](https://github.com/keras-rl/keras-rl/blob/master/rl/policy.py).
        test_policy__: A Keras-rl policy.
        enable_double_dqn__: A boolean which enable target network as a second network proposed by van Hasselt et al. to decrease overfitting.
        enable_dueling_dqn__: A boolean which enable dueling architecture proposed by Mnih et al.
        dueling_type__: If `enable_dueling_dqn` is set to `True`, a type of dueling architecture must be chosen which calculate Q(s,a) from V(s) and A(s,a) differently. Note that `avg` is recommanded in the [paper](https://arxiv.org/abs/1511.06581).
